...
When a measure appears in both KPI and table, reconcile a source-level total for the same date field, period, filters, grain, units, aggregation and NULL policy before presenting it. Formatting cannot reconcile different totals. For budget/cost analysis, align spent, limit, remaining and elapsed-period share to one as-of date; take the limit from the task. Entity breakdowns may overlap, and a current membership lookup does not prove historical membership. Compute unit cost and error rate as ratios of compatible aggregate numerators/denominators. Supply appropriate `metric.precision` for small unit costs so a nonzero cost does not display as zero. Keep unknown/no-data distinct from zero and use only the requested analytical scope.
