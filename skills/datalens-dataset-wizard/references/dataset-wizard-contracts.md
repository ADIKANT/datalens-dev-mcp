...
- On conflict, read back the source IDs and changed content before deciding whether anything applied. For unknown effects, follow [repeat-effect recovery](../../datalens-dashboard/references/authorized-scope.md#unknown-outcomes-and-repeat-effects); changing operation ID does not make a replay safe. A confirmed non-applied outcome permits a corrected in-scope request without another approval conversation.
