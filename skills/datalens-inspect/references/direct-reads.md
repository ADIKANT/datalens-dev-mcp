...
- Use `dl_workbooks_list` or `dl_workbook_entries` for inventory. Continue while `next_page_token` is present; if `complete` is false, call the result partial. To inspect a workbook itself, use `dl_object_get(object_type="workbook", object_id=...)`: its name, description and revision belong to the workbook object. An entry listing describes its contents; even a complete empty listing does not verify workbook metadata or its current revision.
