...
When the user supplies a workbook, read that workbook's card and paginated entry inventory first. Classify entries from actual type/subtype and renderer metadata, distinguishing Wizard, Editor, QL and HTML Page. Keep unknown subtypes explicit and resolve them with addressed reads. An open browser tab, chart name or first project context cannot replace the requested container.
