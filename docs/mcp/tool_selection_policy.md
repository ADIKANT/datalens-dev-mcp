...
Стандартный `tools/list` содержит 38 инструментов. Выбор начинается с цели пользователя, а не с имени низкоуровневого метода.
...
Удаление целого объекта требует `confirm_delete=true` для совпадающего плана. Удаление содержимого внутри объекта остаётся update.
