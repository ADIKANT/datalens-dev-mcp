...
- `delete` — только manifest action `retire_legacy_objects` после отдельного
  подтверждения;
...
Произвольное удаление целого объекта недоступно. Для manifest action
`retire_legacy_objects` первый вызов возвращает
`delete_confirmation_required`, точные ID и hash плана. Выполнение требует
`confirm_delete=true` для того же плана.
