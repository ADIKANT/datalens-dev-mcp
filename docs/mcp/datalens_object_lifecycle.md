...
Удаление целого объекта требует отдельного `confirm_delete=true` для неизменившегося плана с точными ID и связями. Удаление элемента внутри объекта выполняется как update. Перемещение и изменение прав доступа не поддерживаются.
