...
## Быстрая генерация отдельной HTML-страницы

Для самодостаточной HTML-страницы не нужны поиск воркбука, snapshot,
инвентаризация или DataLens API. Используйте один локальный цикл:
...
HTML в MCP-ответе. Публикация остаётся заблокированной, пока публичная DataLens
API не документирует отдельный create/upload метод для HTML-страниц. Ограничения
и sandbox-контракт описаны в [руководстве по HTML-страницам](datalens/html_pages.md).

Промпт:

> Создай локальную самодостаточную HTML-страницу `<PAGE_ID>`: `<ТРЕБОВАНИЕ>`. Используй `html_page` в `dl_generate_editor_bundle`, затем проверь созданный файл через `dl_validate_editor_runtime_contract`. Не выполняй поиск, snapshot или публикацию в DataLens.
...
- KPI с прозрачным background, без border, radius, outline и shadow, со
  значением `31/34` в compact и `34/38` в normal и рекомендуемой высотой
  виджета `96` px;
