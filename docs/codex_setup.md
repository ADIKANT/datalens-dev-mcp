...
После установки пакета или изменения `config.toml` перезапустите Codex app, CLI-сессию или расширение IDE. В новой задаче откройте `/mcp` и убедитесь, что сервер `datalens_dev` подключён и показывает 8 инструментов профиля `autonomous-v2`.
...
Если `dl_auth_probe` возвращает credential blocker, не ищите старую Codex-задачу. Выполните из checkout `scripts/codex_mcp_launch.sh --recover-credentials`, завершите вход `yc`, если он открылся, и продолжайте текущую задачу. Команда сохраняет токен только в канонический env-файл, перечитывает его и повторяет `dl_auth_probe` без вывода секрета.
