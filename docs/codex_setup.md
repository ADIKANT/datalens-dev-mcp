...
После установки пакета или изменения `config.toml` перезапустите Codex app, CLI-сессию или расширение IDE. В новой задаче откройте `/mcp` и убедитесь, что сервер `datalens_dev` подключён и показывает 39 инструментов.
