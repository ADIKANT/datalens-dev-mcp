...
Фоновое обновление не запускает браузер и не читает интерактивный ввод. Если профиль `yc` требует повторной авторизации, выполните `yc iam create-token` в интерактивном терминале, завершите открывшийся вход и повторите вызов MCP. Ссылка с callback на `127.0.0.1` одноразовая: после завершения или остановки породившего её процесса `yc` она больше не работает.
