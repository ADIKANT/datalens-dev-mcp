...
При `DATALENS_ENABLE_TOKEN_REFRESH_ON_401=1` сервер использует настроенную команду `yc iam create-token --no-browser --no-user-output` в двух случаях:
...
Фоновое обновление не запускает браузер и не читает интерактивный ввод. Если профиль `yc` требует повторной авторизации, выполните из текущего checkout каноническую команду:

```bash
scripts/codex_mcp_launch.sh --recover-credentials
```

Она сначала повторяет безопасное фоновое получение токена, а при необходимости запускает `yc init` тем же interpreter/launcher-контуром. Новый токен атомарно сохраняется только в канонический `DATALENS_ENV_FILE`, затем файл перечитывается и выполняется `dl_auth_probe`. Ответ не содержит значение токена; продолжать можно в той же Codex-задаче, искать старую задачу не требуется. Ссылка с callback на `127.0.0.1` одноразовая: после завершения или остановки породившего её процесса `yc` она больше не работает.
