...
Произвольное удаление целого объекта не поддерживается. Только объявленное в
project manifest действие `retire_legacy_objects` использует отдельный
машинный handshake: первый вызов возвращает точные ID, связи и hash плана, второй
передаёт `confirm_delete=true` для того же плана. Изменившийся план
заново сверяется с разрешённым scope. Удаление целого QL-объекта не поддерживается.
