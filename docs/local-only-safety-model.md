...
project manifest действие `retire_legacy_objects` использует отдельное
подтверждение: первый вызов возвращает точные ID, связи и hash плана, второй
передаёт `confirm_delete=true` для того же плана. Изменившийся план
подтверждается заново. Удаление целого QL-объекта не поддерживается.
