...
Public specs SDK v3.0.0 содержат Page create/get/update/delete; публикация использует revision/mode в update, а не отдельный publish endpoint. Доступные действия определяют проверенные фазы adapter и активная инсталляция. Сохраняйте точные target/revision checks, reconciliation квитанции и readback.

Разделяйте локальный artifact/lint, saved state, published state и render в платформенном iframe. Локальный Browser preview не воспроизводит CSP платформы или export через host. Пустая Page не считается исправленной только потому, что content успешно сохранён.
