...
Тест удаления целого объекта выполняется только на специально созданной цели. Первый вызов должен вернуть `delete_confirmation_required`; второй — принять `confirm_delete=true` для того же plan и ID. Отсутствие объекта проверяется отдельным чтением.
