...
| `dl_read_object` | Читает объект известного типа по ID | Получить актуальную сохранённую или опубликованную версию, включая HTML Page | `object_type`, `object_id`, при необходимости branch | Объект или путь к файлу с полным ответом · `чтение API` | [Справочник методов API](https://yandex.cloud/ru/docs/datalens/openapi-ref/) |
...
| Инструмент | Назначение | Когда использовать | Необходимые данные | Результат и класс | Источник |
| --- | --- | --- | --- | --- | --- |
| `dl_validate_editor_runtime_contract` | Проверяет Editor runtime или standalone HTML sandbox | Перед сохранением Editor либо после генерации HTML | Inline-объект/sections либо JSON, JS, HTML или widget-directory в `artifact_paths` | Кэшируемые Editor findings или строгая HTML-проверка · `локальная` | [Editor](https://yandex.cloud/ru/docs/datalens/charts/editor/methods) · [HTML](datalens/html_pages.md) |
...
| `dl_plan_object_create` | Строит план создания поддерживаемого объекта | Для нового дашборда, чарта, HTML Page, датасета или подключения | `object_type`, `payload`, расположение объекта | Выбранный метод, подготовленный payload и блокировки · `локальная` | [API Reference](https://yandex.cloud/ru/docs/datalens/openapi-ref/) |
