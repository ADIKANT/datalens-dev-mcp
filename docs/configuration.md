...
Если `yc` отсутствует в `PATH` MCP-процесса, задайте `DATALENS_YC_BINARY`. `dl_auth_check` показывает `refresh_available`, не раскрывая токен. Фоновое обновление не открывает браузер. Для штатного входа существующего профиля используйте `dl_auth_refresh`: по умолчанию он разрешает внешний системный браузер и ждёт до 120 секунд, после чего проверяет API-доступ. `allow_browser=false` оставляет неинтерактивный режим с лимитом 15 секунд. Токен хранится в памяти процесса; [подробнее о восстановлении доступа](access.md#автоматическое-получение-и-обновление-токена).
