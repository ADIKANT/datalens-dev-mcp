...
Если `yc` отсутствует в `PATH` MCP-процесса, задайте `DATALENS_YC_BINARY`. `dl_runtime_status` показывает `refresh_available`, не раскрывая путь к токену или его значение. Автоматическое обновление запускает `yc iam create-token --no-browser --no-user-output`; повторную интерактивную авторизацию профиля `yc` нужно завершить отдельно в терминале.
