 ...
 */
// Sankey использует отдельные узлы и связи; циклические переходы должны быть устранены в Sources.
...
const __DL_TITLE_CONTRACT = Object.freeze({"display_title":"Диаграмма потоков","family":"sankey_status_flow","hint":"Потоки между узлами с шириной связей по объёму.","issues":[],"mode":"embedded_title","mutual_exclusion":{"native_and_runtime_hint":"forbidden","native_and_runtime_title":"forbidden"},"native_metadata":{"enableHint":false,"hideTitle":true,"hint":"Потоки между узлами с шириной связей по объёму.","title":"Диаграмма потоков"},"ok":true,"route":"editor_advanced","runtime":{"renders_content_label":false,"renders_hint":true,"renders_title":true},"schema_id":"dashboard_title_contract","sha256":"a73cee2bc1d8c3e1bac948b799a86713979104be6158c35dd5758b704a1e70b7"});
...
const invalidReason = parsedRows.some((row) => !row.source || !row.target || !Number.isFinite(row.value) || !(row.value > 0))
  ? 'нужны_source_target_и_положительное_value'
  : '';
const rows = parsedRows.map((row) => ({...row, value: Number.isFinite(row.value) ? row.value : 0}));
const model = {title: 'Flow', rows, invalidReason, hint: 'Для потока нужны явные source, target и положительное value.', theme: themeName(), style: HOUSE_STYLE};
...
module.exports = {
  render: Editor.wrapFn({
    ...
    fn: function(options, data) {
      ...
      if (!data.rows.length) {
        return __dlGenerateProfileHtml(options, `<div style="box-sizing:border-box;width:100%;height:100%;padding:12px;background:${style.colors.surface};color:${style.colors.textMuted};font-family:Inter,Arial,sans-serif;">НЕТ ДАННЫХ О ПОТОКАХ</div>`);
      }
      ...
      if (changed) {
        return __dlGenerateProfileHtml(options, `<div style="box-sizing:border-box;width:100%;height:100%;padding:12px;background:${style.colors.surface};color:${style.colors.textMuted};font-family:Inter,Arial,sans-serif;">N/A · циклический_поток</div>`);
      }
      ...
      return __dlGenerateProfileHtml(options, `<div style="box-sizing:border-box;width:100%;height:100%;padding:0;background:${style.colors.surface};font-family:Inter,Arial,sans-serif;overflow:auto;"><svg data-role="sankey" data-node-count="${nodes.length}" data-link-count="${data.rows.length}" viewBox="0 0 ${width} ${height}" width="100%" height="100%" preserveAspectRatio="xMidYMid meet">${links}${nodeMarkup}</svg></div>`);
    },
  }),
};
