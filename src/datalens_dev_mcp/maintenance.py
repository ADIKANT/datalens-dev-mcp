...
def validate_html_content(content: Any) -> dict[str, Any]:
    if not isinstance(content, str):
        return {"ok": False, "issues": [{"code": "html_must_be_string", "message": "HTML Page content must be one string"}]}
    size = len(content.encode("utf-8"))
    if size > HTML_MAX_BYTES:
        return {"ok": False, "issues": [{"code": "html_too_large", "message": f"HTML content exceeds {HTML_MAX_BYTES} bytes"}], "bytes": size}
