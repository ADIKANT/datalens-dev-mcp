    ...
    fn: function(options, data) {
      ...
      function renderGroupedBar() {
        ...
        return groups.map((group) => `<div style="margin:8px 0 12px;"><b style="font-size:11px;color:${style.colors.textMuted};">${esc(group)}</b>${data.rows.filter((row) => row.group === group).map((row, index) => {
          const width = Math.max(2, (row.value / maxValue) * 100);
          return `<div style="display:grid;grid-template-columns:150px 1fr 54px;gap:8px;align-items:center;margin:4px 0;font-size:12px;color:${style.colors.text};"><span>${esc(row.label)}</span><span style="height:10px;background:${style.colors.surfaceMuted};"><i style="display:block;height:10px;width:${width}%;background:${style.colors.category[index % style.colors.category.length]};"></i></span><b style="text-align:right;">${fmt(row.value)}</b></div>`;
        }).join('')}</div>`).join('');
      }
      ...
      function renderBulletAssignees() {
        return data.rows.filter((row) => Number.isFinite(row.target)).map((row) => {
          ...
          return `<div style="display:grid;grid-template-columns:160px 1fr 58px;gap:8px;align-items:center;margin:8px 0;font-size:12px;color:${style.colors.text};"><span>${esc(row.label)}</span><span style="position:relative;height:14px;background:${style.colors.surfaceMuted};"><i style="display:block;height:14px;width:${width}%;background:${style.colors.primary};"></i><em style="position:absolute;left:${target}%;top:-3px;height:20px;border-left:2px solid ${style.colors.danger};"></em></span><b style="text-align:right;">${fmt(row.value)}</b></div>`;
        }).join('');
      }
      function renderHeatmap() {
        return `<div style="display:grid;grid-template-columns:repeat(3,minmax(90px,1fr));gap:6px;">${data.rows.map((row) => {
      ...
      function renderWaterfall() {
        ...
        return data.rows.map((row) => {
          ...
          return `<div style="display:grid;grid-template-columns:150px 1fr 70px;gap:8px;align-items:center;margin:7px 0;font-size:12px;color:${style.colors.text};"><span>${esc(row.label)}</span><span style="height:12px;background:${style.colors.surfaceMuted};"><i style="display:block;height:12px;width:${width}%;margin-left:${row.value < 0 ? 35 : 0}%;background:${color};"></i></span><b style="text-align:right;">${fmt(running)}</b></div>`;
        }).join('');
      }
      ...
      return Editor.generateHtml(`<div style="box-sizing:border-box;width:100%;height:100%;padding:12px 14px;background:${style.colors.surface};font-family:Inter,Arial,sans-serif;overflow:hidden;">${body || `<div style="color:${style.colors.textSubtle};font-weight:800;">NO DATA</div>`}</div>`);
    },
  }),
};
