...
class RouteSelectionPolicyTests(unittest.TestCase):
    def test_wizard_default_and_explicit_js(self):
        ...
        default = select_route("Make a custom chart")
        explicit_js = select_route("Собери на JS")
    ...
    def test_explicit_wizard_does_not_silently_fallback_to_js(self):
        ...
        decision = select_route("Собери wizard chart")
    ...
    def test_table_prefers_native_table_node(self):
        ...
        decision = select_route("Сделай таблицу с bar cells")

        self.assertEqual(decision.selected_route, "wizard_native")
        self.assertEqual(decision.selected_family, "table_node")
        self.assertEqual(decision.visualization_id, "flatTable")

    def test_canonical_selector_and_kpi_routes(self):
        from datalens_dev_mcp.pipeline.route_selection_policy import select_route

        selector = select_route("Build selector row for Period and Team")
        kpi = select_route("Build KPI indicator")
        ...
        self.assertEqual(kpi.policy, "route_selection_policy")

    def test_semantic_map_does_not_require_literal_wizard(self):
        from datalens_dev_mcp.pipeline.route_selection_policy import select_route

        decision = select_route("Build a map of warehouse locations")
