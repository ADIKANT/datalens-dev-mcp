...
class BaselinePreservationTests(unittest.TestCase):
    ...
    def test_create_plan_adds_creation_necessity_proof(self):
        ...
        plan = create_safe_apply_plan(
            project_root="/tmp/dashboard-contract",
            approved=True,
            actions=[
                {
        ...
        self.assertEqual(proof["schema_id"], "datalens.object-creation-necessity")
...
class WizardVisualDatasetTests(unittest.TestCase):
    ...
    def test_renderer_defaults_satisfy_visual_quality(self):
    ...
    def test_visual_quality_blocks_label_gridline_axis_defaults(self):
        from datalens_dev_mcp.pipeline.visual_quality import validate_visual_quality_contract
        ...
        rules = {finding.rule for finding in result.findings}
        self.assertFalse(result.ok)
        self.assertIn("labels_required", rules)
        self.assertIn("gridlines_default_off", rules)
        self.assertIn("measure_axis_title_default_off", rules)
...
class CleanupAndHandoffTests(unittest.TestCase):
