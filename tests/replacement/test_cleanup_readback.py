...
from test_l08_maintenance import Reader, Deleter


def test_preserved_dependency_reached_through_non_candidate_is_kept():
    reader = Reader()
    reader.relations = {"dash": [{"id": "middle", "type": "wizard_chart"}],
                        "middle": [{"id": "shared", "type": "dataset"}], "shared": []}
...
def test_incomplete_dependency_preview_cannot_be_applied():
    ...
    preview = service.preview([{"object_type": "dataset", "object_id": "shared"}],
                              preserve_roots=[{"object_type": "dashboard", "object_id": "dash"}])
