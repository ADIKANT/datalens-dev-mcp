...
def test_name_only_update_uses_official_rename_without_raw_replace():
    renamed = []
    current = {"id": "synthetic", "type": "table_node", "revId": "r1", "name": "Old", "data": {"unknown": "keep"}}
...
def test_mixed_update_saves_then_renames_and_preserves_partial_outcome(rename_fails):
    ...
    def rename(name):
        ...
        return {"id": "synthetic", "type": "table_node", "name": name, "data": {"prepare": "module.exports=[];"}}

    target = SimpleNamespace(response_snapshot={"id": "synthetic", "type": "table_node", "name": "Old", "data": {}}, rename=rename)
    ...
    client = SimpleNamespace(
        get=SimpleNamespace(editor_chart=lambda **_: target),
    ...
    if rename_fails:
        with pytest.raises(UncertainWriteError, match="content saved"):
            adapter.update("editor_chart", "synthetic", {"id": "synthetic", "type": "table_node", "name": "New", "data": {"prepare": "module.exports=[];"}})
    else:
        result = adapter.update(
            "editor_chart", "synthetic", {"id": "synthetic", "type": "table_node", "name": "New", "data": {"prepare": "module.exports=[];"}}
        )
