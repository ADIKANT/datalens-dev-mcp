...
def test_editor_unsupported_tab_prevents_write(filename):
    writes = []
    builder = SimpleNamespace(build=lambda *args: writes.append(args), meta=lambda _: None,
                              description=lambda _: None)
    ...
    with pytest.raises(ValueError, match="unsupported tab"):
        SdkAdapter(client=client).create({
            "name": "Synthetic", "object_type": "editor_chart", "variant": "advanced-chart_node",
...
def test_supported_editor_tabs_are_forwarded_verbatim():
    received, writes = {}, []
    builder = SimpleNamespace(**{name: lambda text, name=name: received.update({name: text})
...
def test_static_validation_does_not_approve_unhandled_config_tab():
    ...
    result = validate_editor_draft({"variant": "advanced-chart_node", "tabs": {
        "meta.json": "{}", "params.js": "", "sources.js": "", "prepare.js": "", "controls.js": "",
        "config.json": "{}"}})
