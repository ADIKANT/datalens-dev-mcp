/*
 * Protected standard_dashboard renderer.
 *
 * This runtime owns layout, formatting, responsive behavior, marks, and
 * tooltips for registered families. This file has no user-editable points:
 * bind a dataset in Meta/Sources and keep its output
 * aliases stable. Extend this runtime only as a reviewed server-level change.
 */
// @cookbook-locale ru Защищённый runtime отвечает за геометрию, tooltip и адаптацию к размеру.
// @cookbook-locale en The protected runtime owns geometry, tooltip, and size adaptation.

/** @typedef {Object} StandardSourceRow Named source row keyed by stable Sources aliases. */
/** @typedef {Object} StandardChartModel Registered family data passed to the protected renderer. */
/**
 * @typedef {Object} StandardRenderOptions
 * @property {number} [width] Actual widget width supplied by DataLens.
 * @property {number} [height] Actual widget height supplied by DataLens.
 */

/** Normalize DataLens metadata/row events into named objects.
 * @param {Object} loadedData Editor.getLoadedData() result.
 * @param {string} sourceName Sources export name.
 * @returns {StandardSourceRow[]} Named rows.
 */
...
// Numeric and date normalization helpers never invent missing business values.
...
/** Convert a source value to a finite number without replacing missing data.
...
// Period and comparison helpers keep current and comparison windows explicit.
...
// Model builders transform normalized rows into renderer-owned chartData only.
...
/** Create the responsive DataLens render export for one chartData model.
 * @param {StandardChartModel} chartData Registered renderer model.
 * @returns {object} Editor.wrapFn render descriptor.
 */
function createRender(chartData) {
  return Editor.wrapFn({
    args: [chartData],
    /** Render one responsive frame.
     * @param {StandardRenderOptions} options Current widget dimensions.
     * @param {StandardChartModel} data Normalized registered-family model.
     * @returns {string} DataLens-safe generated HTML.
     */
...
/** Export the paired renderer and native tooltip contract.
 * @param {StandardChartModel} chartData Registered renderer model.
 * @returns {Object} DataLens Editor exports.
 */
      ...
      renderer: Editor.wrapFn({
        ...
        fn: function(event, data) {
          ...
          function grainScope(value, source = data) {
            ...
            const label = labels[grain] || 'PERIOD';
