...
| Создать или обновить | Проверенный payload для дашборда, чарта, датасета или подключения |
...
```text
Создай локальную self-contained HTML-страницу <PAGE_ID>: <ТРЕБОВАНИЕ>. Проверь sandbox-контракт. Не пытайся загружать её в DataLens.
```
...
- создание и обновление поддерживаемых дашбордов, чартов, датасетов и подключений через plan и Safe Apply;
...
- локальная генерация self-contained HTML artifacts и проверка sandbox-контракта;
...
- Standalone HTML создаётся локально и не загружается через DataLens Public API.
...
- Источник точного набора инструментов — `tools/list` текущей установленной версии; стандартная поверхность версии 0.4.0 содержит 39 инструментов.
